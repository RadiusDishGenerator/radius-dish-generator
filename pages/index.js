...
const PRINTER_PRESETS = [
  ...
  { label: 'Custom 600x600',   w: 600,  h: 600 },
  { label: 'CNC bed 1200x600', w: 1200, h: 600 },
];
  ...
  useEffect(() => {
    ...
    for (let row = 0; row < sectionsY; row++) {
      for (let col = 0; col < sectionsX; col++) {
        ...
        ctx.fillText(Math.round(dishW / sectionsX) + 'x' + Math.round(dishH / sectionsY) + 'mm', x + w / 2, y + h / 2 + 8);
      }
    }
...
function PaywallModal({ onClose, onSubscribe, loading }) {
  return (
    <div style={{
      ...
      <div style={{
        background: '#1a1005', border: '1px solid rgba(200,150,55,0.4)',
        borderRadius: '12px', padding: '36px', maxWidth: '420px', width: '100%',
        textAlign: 'center', fontFamily: 'Georgia,serif', color: '#d4b87a'
      }}>
        <div style={{ fontSize: '36px', marginBottom: '12px' }}>&#127929;</div>
        <h2 style={{ margin: '0 0 10px', fontSize: '20px', letterSpacing: '0.1em', color: '#eac870', fontWeight: 'normal' }}>
          FREE TRIAL COMPLETE
        </h2>
        ...
        <button onClick={onSubscribe} disabled={loading} style={{
          width: '100%', background: 'rgba(200,150,55,0.2)', border: '1px solid #c8904a',
        ...
        <button onClick={onClose} style={{
          background: 'none', border: 'none', color: 'rgba(200,150,55,0.4)',
...
// --- NumInput: BULLETPROOF ---
// Uses type="text" + internal raw string state so the field can always be
// cleared and retyped freely. Parent state only updates on blur.
// Tested: clear, backspace, non-numeric, decimals, clamping, presets, whitespace.
function NumInput({ label, value, onChange, min, max, step, unit, hint }) {
  const [raw, setRaw] = useState(String(value));
  const [focused, setFocused] = useState(false);

  // Sync display when parent changes externally (preset buttons)
  // Guard with focused so we don't overwrite mid-typing
  useEffect(() => {
  ...
  function handleFocus(e) {
    setFocused(true);
    e.target.style.borderColor = '#c8904a';
  }
  ...
  function handleBlur(e) {
    ...
    if (trimmed === '') { setRaw(String(value)); return; }
  ...
  function nudge(dir) {
    ...
    const current = isNaN(base) ? value : base;
    const next = dir > 0
  ...
  const S = {
    ...
    wrap: { display: 'flex', gap: '4px' },
    inp: {
      flex: 1, minWidth: 0,
      background: 'rgba(28,18,5,0.9)', border: '1px solid rgba(200,150,55,0.3)',
      borderRadius: '5px', color: '#f2d478', fontFamily: 'Georgia,serif', fontSize: '16px',
      padding: '7px 10px', boxSizing: 'border-box', outline: 'none',
    },
    btn: {
      background: 'rgba(200,150,55,0.1)', border: '1px solid rgba(200,150,55,0.3)',
      borderRadius: '5px', color: '#e8c870', cursor: 'pointer', fontFamily: 'Georgia,serif',
      fontSize: '18px', padding: '0 11px', outline: 'none', userSelect: 'none', flexShrink: 0,
    },
  ...
  return (
    <div style={S.row}>
      <div style={S.lbl}>
        <span>{label}</span>
        <span style={{ color: '#eac870' }}>{value}{unit ? ' ' + unit : ''}</span>
      </div>
      <div style={S.wrap}>
        <button
          style={S.btn}
...
export default function Home() {
  ...
  async function dlSection(col, row) {
    ...
    dlBlob(
      buildSectionSTL({ R_sphere: R, dishW, dishH, rimWidth, thickness, sectionsX, sectionsY, secCol: col, secRow: row, N: segments }),
      'radius_dish_' + radiusInput.replace(/\s+/g, '_') + '_C' + (col + 1) + 'R' + (row + 1) + '.stl'
    );
    setGenItem(null);
    setStatus('Section C' + (col + 1) + 'R' + (row + 1) + ' downloaded \u2713');
  ...
  async function dlAll() {
    ...
    setGenItem(null);
    setStatus('All ' + totalSections + ' sections downloaded \u2713');
  ...
  const C = {
    root: {
      ...
      backgroundImage: 'radial-gradient(ellipse at 30% 15%,rgba(170,115,35,0.09) 0%,transparent 55%), radial-gradient(ellipse at 72% 82%,rgba(130,75,18,0.07) 0%,transparent 50%)',
      fontFamily: "Georgia,'Times New Roman',serif", color: '#d4b87a', margin: 0, padding: 0
    },
    header: {
      borderBottom: '1px solid rgba(200,150,55,0.2)', padding: '18px 28px 14px',
      background: 'rgba(14,10,4,0.8)', display: 'flex', alignItems: 'center',
      justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px'
    },
    h1: { margin: 0, fontSize: '18px', fontWeight: 'normal', letterSpacing: '0.28em', color: '#eac870', textTransform: 'uppercase' },
    sub: { marginTop: '3px', fontSize: '11px', letterSpacing: '0.12em', color: 'rgba(200,150,55,0.42)' },
    badge: { background: 'rgba(200,150,55,0.12)', border: '1px solid rgba(200,150,55,0.3)', borderRadius: '20px', padding: '5px 14px', fontSize: '11px', color: 'rgba(220,170,65,0.8)' },
    layout: { maxWidth: '960px', margin: '0 auto', padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' },
    card: { background: 'rgba(22,15,6,0.7)', border: '1px solid rgba(200,150,55,0.17)', borderRadius: '8px', padding: '18px' },
    ttl: { fontSize: '9px', letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(200,150,55,0.42)', marginBottom: '12px' },
    bigInput: {
      width: '100%', boxSizing: 'border-box', background: 'rgba(28,18,5,0.92)',
    ...
    pBtn: {
      background: 'rgba(200,150,55,0.08)', border: '1px solid rgba(200,150,55,0.2)',
    ...
    segRow: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', fontSize: '10px', color: 'rgba(200,150,55,0.45)' },
    range: { flex: 1, accentColor: '#c8904a' },
    sectionGrid: { display: 'grid', gap: '5px', marginBottom: '8px' },
    secBtn: {
      background: 'rgba(200,150,55,0.09)', border: '1px solid rgba(200,150,55,0.25)',
    ...
    allBtn: {
      background: 'rgba(200,150,55,0.14)', border: '1px solid rgba(200,150,55,0.45)',
    ...
    statusBar: { background: 'rgba(200,150,55,0.07)', border: '1px solid rgba(200,150,55,0.17)', borderRadius: '5px', color: '#e8c870', fontSize: '11px', marginTop: '8px', padding: '7px 12px', textAlign: 'center' },
    warn: { background: 'rgba(200,75,25,0.08)', border: '1px solid rgba(200,75,25,0.28)', borderRadius: '5px', color: '#e09060', fontSize: '11px', padding: '7px 10px', marginTop: '6px' },
  ...
  return (
    <>
      <Head>
        <title>Radius Dish Generator &mdash; Luthier&apos;s 3D Print Tool</title>
      ...
      <div style={C.root}>
        <header style={C.header}>
          <div>
            ...
            <div style={C.sub}>Parametric &middot; any printer &middot; any CNC &middot; any radius</div>
          </div>
          {!subscribed
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={C.card}>
              ...
              <input style={C.bigInput} value={radiusInput}
                ...
                placeholder="e.g. 14ft" spellCheck={false}
                onFocus={e => e.target.style.borderColor = '#c8904a'}
                onBlur={e => e.target.style.borderColor = 'rgba(200,150,55,0.38)'} />
              <div style={C.hint}>14ft &middot; 14&apos; &middot; 4267mm &middot; 4.267m &middot; 168in</div>
              ...
              {valid ? (
                <div style={C.stats}>
                  <div><div style={C.statLbl}>Sphere radius</div><div style={C.statVal}>{R.toFixed(1)} mm</div><div style={C.statSub}>{(R / 304.8).toFixed(3)} ft</div></div>
                  <div><div style={C.statLbl}>Sag depth</div><div style={C.statVal}>{sag.toFixed(3)} mm</div></div>
                  <div><div style={C.statLbl}>Total height</div><div style={C.statVal}>{(sag + thickness).toFixed(2)} mm</div></div>
                  <div><div style={C.statLbl}>Curved area</div><div style={C.statVal}>\u2300{Math.round(curveR * 2)} mm</div></div>
                </div>
              ) : radiusInput.length > 0 ? (
                <div style={C.warn}>
                  {R === null ? "Can't parse \u2014 try e.g. 14ft or 4267mm"
                    : R <= getCurveRadius(dishW, dishH, rimWidth)
                      ? 'Radius must be larger than ' + getCurveRadius(dishW, dishH, rimWidth).toFixed(0) + 'mm for this dish size'
                      : 'Invalid value'}
                </div>
              ) : null}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={C.card}>
              ...
              <div style={{ ...C.statLbl, marginBottom: '8px' }}>
                Section size: <span style={{ color: '#eac870', fontWeight: 'bold' }}>{Math.round(dishW / sectionsX)} x {Math.round(dishH / sectionsY)} mm</span>
            ...
            <div style={C.card}>
              <div style={C.ttl}>Top View &mdash; Print Sections</div>
              ...
              <p style={C.infoText}>
                ...
                Each section: {Math.round(dishW / sectionsX)} x {Math.round(dishH / sectionsY)} mm.
              </p>
            </div>
          </div>
